.SILENT: anubis-vault-single-encrypt
.PHONY: anubis-vault-single-encrypt
anubis-vault-single-encrypt: anubis-vault-single-encrypt
	- $(call print_running_target)
	- $(eval name=$(@:%-encrypt=%))
	- $(call print_running_target, encrypting single file '${ARTIFACTS_ROOT}/no-extension')
	- $(eval command=bin/anubis-vault encrypt)
ifneq (${LOG_LEVEL}, )
...
.SILENT: anubis-vault-multi-encrypt
.PHONY: anubis-vault-multi-encrypt
anubis-vault-multi-encrypt: 
	- $(call print_running_target)
ifneq ($(FILE_COUNT),)
	- $(eval name=$(@:%-encrypt=%))
	...
	- $(eval command=$(command) && bin/anubis-vault encrypt)
ifneq (${LOG_LEVEL}, )
	- $(eval command=$(command) --log-level=$(LOG_LEVEL))
endif
ifneq (${ENCRYPTION_KEY_FILE}, )
	...
	- $(eval command=$(command) --regex='*.${FILE_EXTENSION}'  > /tmp/$(name).json)
	- @$(MAKE) --no-print-directory -f $(THIS_FILE) shell cmd="${command}"
ifneq ($(DELAY),)
	- sleep $(DELAY)
endif
