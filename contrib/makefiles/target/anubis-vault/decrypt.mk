.SILENT: anubis-vault-single-decrypt
.PHONY: anubis-vault-single-decrypt
anubis-vault-single-decrypt: anubis-vault-single-encrypt
	...
	- $(eval command=bin/anubis-vault decrypt)
ifneq (${LOG_LEVEL}, )
...
.SILENT: anubis-vault-multi-decrypt
.PHONY: anubis-vault-multi-decrypt
anubis-vault-multi-decrypt: anubis-vault-multi-encrypt
	- $(call print_running_target)
ifneq ($(FILE_COUNT),)
	- $(eval name=$(@:%-decrypt=%))
	- $(eval NONCE=$(shell jq -c '.[].random_nonce'  /tmp/$(name).json))
